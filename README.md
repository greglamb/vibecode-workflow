...
| Command | Description |
|---------|-------------|
| `/check` | Runs unit and integration tests, validates code against project standards |
| `/commit` | Diffs changes, generates a [Conventional Commits](https://www.conventionalcommits.org/) message with [Gitmoji](https://gitmoji.dev/), stages, and commits |
| `/next` | Asks Claude what the next logical step is |
| `/plot` | Syncs episodic memory, loads superpowers, then plans using brainstorm + extension skills |
| `/todo` | Prints the current `TODO.md` contents |
